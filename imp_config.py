...
from openerp.tools import file_open #, ustr
import urllib2
...
class imp_config(models.Model):
#class imp_config(osv.osv):

    _name = 'imp.config'
    name = fields.Char(string='Name')
    import_fields_types = fields.Text(string="Fields types") 
    data_type = fields.Many2one(comodel_name='ir.model', string='Model to import')    
    ...
    #_columns = {
        ...
        #'data_type': fields.many2one('ir.model', string='Model to import')
    #}
   ...
   # _auto = True
    ...
    @api.multi
    def get_prod_fields(self):
#    def get_prod_fields(self, cr, uid, ids, context={}):

        inst = self
        sel_mod = self.data_type
        fields = self.env['crm.lead'].fields_get()
        #fields = imp_mod_env_inst.fields_get()
...
#        imp_model_id = self.pool.get('ir.model').search(cr, 1, [('model','=','imp.config')])[0]
        imp_model_id = self.env['ir.model'].search([('model','=','imp.config')])[0].id
        form_env = self.env['ir.ui.view']
        imp_form_id = form_env.search([('name','=','imp.config.form')])[0]
#        imp_form_id = self.pool.get('ir.ui.view').search(cr, 1, [('name','=','imp.config.form')])[-1]
        imp_form = imp_form_id
        _mylog.info('imp form: %s' % (unicode(imp_form)))
        arch = imp_form.arch
        st = ''
        end = '''</group>
</form>'''
        ...
        for field in keys:
            if fields[field]['type'] in allowed_types and field not in restricted_fields:
                ...
                exist = self.env['ir.model.fields'].search([('name','=','x_'+ field),('model_id','=',imp_model_id)])
#                exist = self.pool.get('ir.model.fields').search(cr, 1, [('name','=','x_'+ field),('model_id','=',imp_model_id)])
 ...
 #                   new = self.pool.get('ir.model.fields').create(cr, 1, {'name':'x_bool_'+ field,
                    new = self.env['ir.model.fields'].create({'name':'x_bool_'+ field,
                                                                     #   'model_id':self.id,
                ...
                if ('x_bool_'+field) in arch:
                    ...
                    _mylog.info('added to the view: %s' % (unicode('''<field name="x_bool_%s" colspan="3" />\n''' % (field,))))
        if st:
            arch = arch.replace(end, st+end)
            
            imp_form.write({'arch': arch})
#            self.env['ir.ui.view'].write(imp_form_id.id, {'arch': arch})
...
#        self_fields = self.pool.get('imp.config').fields_get(cr,uid)
        ...
        fields = self.pool.get('crm.lead').fields_get(cr,1)
#        fields = self.pool.get(inst.data_type).fields_get(cr,1)
#        fields = self.pool.get('product.proxy').fields_get(cr,1)
